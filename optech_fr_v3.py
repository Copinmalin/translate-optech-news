...
def fetch(url: str) -> requests.Response:
    headers = {
        "User-Agent": "optech-fr-md/3.0 (+https://bitcoinops.org)"
    }
...
def translate_title(en_title: str) -> str:
    title = en_title
    title = title.replace("Bitcoin Optech Newsletter", "Bulletin Hebdomadaire Bitcoin Optech")
...
def translate_body_with_openai(body_markdown: str, source_url: str, model: str) -> str:
    client = OpenAI()

    system_prompt = """Tu traduis des fichiers source Markdown/Jekyll de Bitcoin Optech de l'anglais vers le français.

Règles impératives :
- Retourne uniquement le corps Markdown, sans front matter YAML.
- Préserve strictement la structure Markdown.
...
- Ne modifie pas les noms de référence en bas du fichier, uniquement le texte visible au-dessus si nécessaire.
- Conserve les backticks, emphases, listes, titres et ancres.
- Traduis le texte visible en français naturel et technique.
- N'invente rien, ne résume rien, ne coupe rien.
"""

    user_prompt = f"""Source : {source_url}

Traduis ce corps Markdown en français :
...
def process_one(
    ...
    try:
        ...
        translated_body = translate_body_with_openai(body, source_url, model)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Traduit en français les newsletters Bitcoin Optech depuis le Markdown source du dépôt upstream."
    )
    parser.add_argument(
        "target",
