...
def candidate_fr_paths(en_path_no_anchor: str) -> list[str]:
    candidates = []
    if en_path_no_anchor.startswith("/en/"):
        direct = "/fr/" + en_path_no_anchor[len("/en/"):]
        candidates.append(direct)
        if "/newsletters/" in direct:
            candidates.append(direct.replace("/newsletters/", "/bulletins/", 1))
        if "/bulletins/" in direct:
            candidates.append(direct.replace("/bulletins/", "/newsletters/", 1))
...
def resolve_internal_url(en_relative_url: str) -> str:
    ...
    en_exists, _, en_ids = fetch_page(en_path)
    if not en_exists:
    ...
    chosen_fr_path = None
    fr_ids = []
    for fr_path in candidate_fr_paths(en_path):
        fr_exists, _, ids = fetch_page(fr_path)
        if fr_exists:
            chosen_fr_path = fr_path
            fr_ids = ids
            break

    if chosen_fr_path is None:
        _resolution_log[en_relative_url] = {"status": "kept_en", "reason": "fr_missing"}
        return en_relative_url
