...
def fetch(url: str) -> requests.Response:
    headers = {
        "User-Agent": "optech-fr-batch/1.0 (+https://bitcoinops.org)"
    }
...
def extract_article_fragment(html: str, base_url: str) -> tuple[str, str]:
    ...
    for sib in h1.find_next_siblings():
        if getattr(sib, "name", None) in {"footer", "nav"}:
            break
    ...
    for tag in fragment.select("script, style, form, aside"):
        tag.decompose()

    for a in fragment.find_all("a", href=True):
        a["href"] = urljoin(base_url, a["href"])

    return title, str(fragment)
...
def translate_with_openai(article_title: str, article_html: str, source_url: str, model: str) -> str:
    client = OpenAI()
...
Traduis ce contenu HTML en français, en Markdown :
...
def process_one(
    ...
    try:
        if not force_gpt:
            ...
            try:
                ...
                if looks_like_valid_fr_newsletter(fr_resp.url, fr_resp.text):
                    title, article_html = extract_article_fragment(fr_resp.text, fr_resp.url)
        ...
        title, article_html = extract_article_fragment(en_resp.text, en_resp.url)
        output_path = build_output_path(output_dir, en_url, title)
        ...
        markdown = translate_with_openai(title, article_html, en_resp.url, model)
        save_markdown(output_path, markdown)
