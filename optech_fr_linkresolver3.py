...
def common_indent(lines: list[str]) -> str:
    indents = []
    for line in lines:
        if line.strip():
            m = re.match(r"^(\s*)", line)
...
def wrap_markdown_body(text: str, width: int) -> str:
    ...
    def flush_plain():
        nonlocal buf
        if buf:
            indent = common_indent(buf)
            out.append(wrap_paragraph(" ".join(s.strip() for s in buf), width, initial_indent=indent, subsequent_indent=indent))
            buf = []
    ...
    while i < len(lines):
        ...
        if is_list_item(line):
            ...
            while j < len(lines):
                ...
                continuation_source.append(nxt)
                j += 1
            continuation_indent = common_indent(continuation_source) if continuation_source else (" " * len(indent))
            out.append(wrap_paragraph(" ".join(item_lines), width, initial_indent=indent, subsequent_indent=continuation_indent or (" " * len(indent))))
            i = j
            continue
